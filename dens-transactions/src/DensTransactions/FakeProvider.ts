...
export const mkOgmiosCxt = (url: string, port: number): Promise<OGM.InteractionContext> => {
...
export const containsAssetClass = (value: Schema.Value, unit: Unit) : boolean => {
...
export class OgmiosOnly implements Provider {
  ...
  async getProtocolParameters(): Promise<ProtocolParameters> {
    ...
    return {
      ...
      minFeeB: parseInt((params.minFeeConstant as unknown as Lovelace).lovelace),
      maxTxSize: params.maxTransactionSize.bytes,
      maxValSize: params.maxValueSize.bytes,
      keyDeposit: BigInt(parseInt((params.stakeCredentialDeposit as unknown as Lovelace).lovelace)),
      poolDeposit: BigInt(parseInt((params.stakePoolDeposit as unknown as Lovelace).lovelace)),
  ...
  async getUtxos(addressOrCredential: Address | Credential): Promise<UTxO[]> {
    if (typeof addressOrCredential === 'string') {
      ...
      return txs.map(x => toLucidUtxo(x));
       ...
       const credentialAddr = credentialToAddress(this.network,addressOrCredential);
  ...
  async getUtxosWithUnit(
    ...
    if (typeof addressOrCredential === 'string') {
      ...
      return txs.filter(x => containsAssetClass(x.value,unit)).map(x => toLucidUtxo(x));
    } else {
       const credentialAddr = credentialToAddress(this.network,addressOrCredential);
  ...
  async getUtxoByUnit(unit: Unit): Promise<UTxO> {
    ...
    return txs.filter(x => containsAssetClass(x.value,unit)).map(x => toLucidUtxo(x))[0];
